...
?>
            ...
            <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=183ddc7a4229c31fa605b00c1f7c35c4&action=lists&catid=%24catid&num=18&order=listorder+DESC&page=%24page\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'lists')) {$pagesize = 18;$page = intval($page) ? intval($page) : 1;if($page<=0){$page=1;}$offset = ($page - 1) * $pagesize;$content_total = $content_tag->count(array('catid'=>$catid,'order'=>'listorder DESC','limit'=>$offset.",".$pagesize,'action'=>'lists',));$pages = pages($content_total, $page, $pagesize, $urlrule);$data = $content_tag->lists(array('catid'=>$catid,'order'=>'listorder DESC','limit'=>$offset.",".$pagesize,'action'=>'lists',));}?>
            <?php
...
$odd = $i % 2 == 0 ? "odd" : "even";
?>
            <div style="height:108px;"class="zbjj2 chunk_<?php echo $catid;?>_<?php echo $i;?>">


            </div>

            <div style="width: 900px;" class="iblock three_2-1 three_<?php echo $odd;?>">
                <?php
                $j = 1;
                ?>
                <?php $n=1;if(is_array($r)) foreach($r AS $rr) { ?>
                <div class="three_<?php echo $j;?> b"><a href="<?php echo $rr['url'];?>"><img src="<?php echo $rr['thumb'];?>" style="width: 100%"></a></div>
                <?php
                    $j++;
                ?>
                <?php $n++;}unset($n); ?>

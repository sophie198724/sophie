...
<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=cbb2c85847c03b60359ad06314968256&sql=select+%2A+from+v9_category+where+parentid%3D%24catid+order+by+catid+desc+\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("select * from v9_category where parentid=$catid order by catid desc  LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
<?php if($data) { ?>
<?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
  <?php
    $catid = $r['catid'];
  ?>
  <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"get\" data=\"op=get&tag_md5=181daed4488491d1001192d62048afbb&sql=select+%2A+from+v9_page+where+catid%3D%24catid\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}pc_base::load_sys_class("get_model", "model", 0);$get_db = new get_model();$r = $get_db->sql_query("select * from v9_page where catid=$catid LIMIT 20");while(($s = $get_db->fetch_next()) != false) {$a[] = $s;}$data = $a;unset($a);?>
  <?php $n=1;if(is_array($data)) foreach($data AS $r) { ?>
    <?php
      $content = $r['content'];
      $title = $r['title'];
...
<div id="content">
	...
	<div class="col-left left-nav">
    	<h4 style="background: url(<?php echo IMG_PATH;?>55_left.jpg) no-repeat left top #b3291e !important;
    line-height: 35px;
    padding: 15px 5px;
    font-weight: bold;
    text-align: center;font-size: 20px;
    color: #fff;"><?php echo $CATEGORYS[$CAT['parentid']]['catname'];?> </h4><br/>
    	<ul class="content">
        	<?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=c162d98c554b70af815cf67758519840&action=category&catid=82&num=12&siteid=%24siteid&order=listorder+ASC\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'category')) {$data = $content_tag->category(array('catid'=>'82','siteid'=>$siteid,'order'=>'listorder ASC','limit'=>'12',));}?>
			<?php $n=1;if(is_array($data)) foreach($data AS $c) { ?>
			   <li><a <?php if($catid == $c[catid]) { ?> class="on"  style="background-color:#b3291e;color:#fff;"<?php } ?> href="<?php echo $c['url'];?>" ><?php echo $c['catname'];?></a></li>
  ...
  <div class="col-auto">
    	...
    	<DIV class=nymore>当前位置<a href="<?php echo siteurl($siteid);?>">首页</a><span> > </span><?php echo catpos($catid);?></DIV>
        <div class="content">

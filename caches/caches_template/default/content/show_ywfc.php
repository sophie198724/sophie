...
<div id="content">
    <div class="col-left left-nav">
        ...
        <ul class="content">
            <?php if(defined('IN_ADMIN')  && !defined('HTML')) {echo "<div class=\"admin_piao\" pc_action=\"content\" data=\"op=content&tag_md5=b361e3ff2985d2f0b621f3571523f4f0&action=category&catid=187&num=12&siteid=%24siteid&order=listorder+ASC\"><a href=\"javascript:void(0)\" class=\"admin_piao_edit\">编辑</a>";}$content_tag = pc_base::load_app_class("content_tag", "content");if (method_exists($content_tag, 'category')) {$data = $content_tag->category(array('catid'=>'187','siteid'=>$siteid,'order'=>'listorder ASC','limit'=>'12',));}?>
            <?php $n=1;if(is_array($data)) foreach($data AS $c) { ?>
            <li><a  <?php if($catid == $c[catid]) { ?> class="on" style="background-color:#29166f;color:#fff;" <?php } ?>  href="<?php echo $c['url'];?>"><?php echo $c['catname'];?></a>
                <!--<ul>
    ...
    <div class="col-auto">
        <DIV class=nymore>当前位置：首页 ><?php echo catpos($catid);?><!--<a href="<?php echo siteurl($siteid);?>">首页</a> &gt;<a href="<?php echo $CATEGORYS['144']['url'];?>"><?php echo $CATEGORYS['144']['catname'];?></a> &gt; <?php echo $CATEGORYS[$catid]['catname'];?>--> </DIV>

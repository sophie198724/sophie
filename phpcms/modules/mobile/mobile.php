...
class mobile extends admin {
	...
	public function mobile_update(){
		...
		$a=$db->select(array('module'=>'content','type'=>1,'siteid'=>$this->get_siteid()));
		foreach($a as $k=>$v){
			$data['catid']=$v['catid'];
			...
			$data['type']=$v['type'];
			$data['url']=APP_PATH.'index.php?m=mobile&a=page&catid='.$v['catid'].'&siteid='.$v['siteid'];
		...
		$b=$db->select(array('module'=>'content','type'=>0,'siteid'=>$this->get_siteid()));
		foreach($b as $k=>$v){
			$data['catid']=$v['catid'];
			$data['parentid']=$v['parentid'];
			$data['catname']=$v['catname'];
			...
			$data['listorder']=$v['listorder'];
			$data['type']=$v['type'];
			$data['url']=APP_PATH.'index.php?m=mobile&a=lists&catid='.$v['catid'].'&siteid='.$v['siteid'];
			$check=$this->cate_db->get_one(array('catid'=>$v['catid']));
			if(!$check){
			$this->cate_db->insert($data,true);
			}else{
